...
/**
 * DSL-style select().from().innerJoin()...
 * 
 * @author Alvin
 */
public class HJoin {
   ...
   public HJoin(HQueryInfo query, String joinType, String join) {
      this.query = query;
      this.query.addJoin(joinType, join, null);
   }

   public HJoin(HQueryInfo query, String joinType, Class<?> join) {
      this(query, joinType, join.getName());
   }

   public HJoinAs as(String name) {
      return new HJoinAs(this.query, name);
   }
