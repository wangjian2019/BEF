...
/**
 * DSL-style select().from().orderBy().desc()...
 * 
 * @author Alvin
 */
public class HOrderByDesc {

   final HQueryInfo query;

   public HOrderByDesc(HQueryInfo query) {
      this.query = query;
      this.query.setOrderByDesc();
   }

   public HOrderBy orderBy(String orderBy) {
      return new HOrderBy(this.query, orderBy);
   }
